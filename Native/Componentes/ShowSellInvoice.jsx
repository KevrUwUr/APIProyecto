          ...
          {items.map((item) => (
            <TouchableOpacity key={item.key} onPress={() => handleItemPress(item)}>
              <DataTable.Row>
                ...
                <DataTable.Cell dateTime>{item.expeDate}</DataTable.Cell>
                <DataTable.Cell dateTime>{item.expiDate}</DataTable.Cell>
      ...
      >
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          <View style={{ backgroundColor: 'white', padding: 20, borderRadius: 10 }}>
            {selectedItem && (
              <>
                <Text>Num: {selectedItem.num}</Text>
                <Text>ExpeDate: {selectedItem.expeDate}</Text>
                <Text>ExpiDate: {selectedItem.expiDate}</Text>
...
export default ShowSellInvoice;
