...
namespace Service
{
    internal sealed class ContactoUsuarioService : IContactoUsuarioService
    {
        ...
        public (ContactoUsuarioForUpdateDTO contUsuarioToPatch, ContactoUsuario contUsuarioEntity) GetContactoUsuarioForPatch(Guid usuarioId, Guid id, bool catTrackChanges, bool prodTrackChanges)
        {
            var usuario = _repository.Usuario.GetUser(usuarioId, catTrackChanges);
            if (usuario is null)
                throw new UsuarioNotFoundException(usuarioId);

            var contUsuarioEntity = _repository.ContactoUsuario.GetContactUserForUser(usuarioId, id,
            prodTrackChanges);
